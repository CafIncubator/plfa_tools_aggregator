...
class Aggregator():
    ...
    Attributes:
        transform_raw_to_stacked (self, plfa_tools_output: pd.DataFrame): Reformats output from PLFA Tools, of a single sample, into proper tabular format
        transform_stacked_to_tidy (self, df: pd.DataFrame): Accepts a Pandas DataFrame generated by "read_dir" or "read_file" and returns a new DataFrame in a tidy format
        read_file (self, file_path: pathlib.Path): Reads a file from PLFA Tools and returns a pandas DataFrame with all data from all worksheets
        read_dir (self, dir_path: pathlib.Path): Reads a directory path that contains multiple files from PLFA Tools and returns a pandas DataFrame with all data from all files and worksheets
    """
    ...
    def transform_raw_to_stacked(self, plfa_tools_output: pd.DataFrame) -> pd.DataFrame:
        ...
        :return: DataFrame. Header names: SampleId, GCRunId, GCFileLoc, ProcessingCode, ProcessMethod, RunDateTime, RT, Response, Ar/Ht, RFact, ECL, Peak Name, Percent, Comment1, Comment2
        """
    ...
    def transform_stacked_to_tidy(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        :return: DataFrame. Headers names: SampleId, GCRunId, GCFileLoc, ProcessingCode, ProcessMethod, RunDateTime, RT, Response, Ar/Ht, RFact, ECL, Peak Name, Percent, Comment1, Comment2
        ...
        peak_names = ['General FAME', 'AM Fungi', 'Gram Negative', 'Eukaryote', 'Fungi', 'Gram Positive', 'Anaerobe', 'Methanotroph', 'Actinomycetes', 'Protozoa']
        ...
        pivot_peaks = measurement_df.pivot(index='SampleID', columns='Peak Name', values='Response')
    ...
    def read_file(self, file_path: pathlib.Path) -> pd.DataFrame:
        ...
        :return: DataFrame. A tidy version of Dataframe given by the filepath
        """
    ...
    def read_dir(self, dir_path: pathlib.Path) -> pd.DataFrame:
        ...
        :return: DataFrame. One or more Tidy DataFrames appended together into one DataFrame
        """
